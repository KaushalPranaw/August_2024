...
public class DP12_MaximumFallingPathSum {
    ...
    /*private static int getMaxPathSum(int[][] matrix) {
        ...
        for (int j = 0; j < n; j++) {
            maxi = Leetcode.Math.max(maxi, getMaxPathSumUtil(m - 1, j, n, matrix, dp));
        }
        return maxi;
    }
    ...
    private static int getMaxPathSumUtil(int i, int j, int n, int[][] matrix, int[][] dp) {
        ...
        return dp[i][j] = Leetcode.Math.max(up, Leetcode.Math.max(leftDiagonal, rightDiagonal));
    }*/
    ...
    private static int getMaxPathSum(int[][] matrix) {
        ...
        for (int i = 1; i < m; i++) {
            for (int j = 0; j < n; j++) {
                ...
                dp[i][j] = Leetcode.Math.max(up, Leetcode.Math.max(leftDiagonal, rightDiagonal));
            }
        }
        ...
        int maxi = Integer.MIN_VALUE;
        for (int j = 0; j < n; j++) {
            maxi = Leetcode.Math.max(maxi, dp[m - 1][j]);
        }
