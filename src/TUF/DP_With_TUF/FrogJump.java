...
public class FrogJump {
    ...
    static int frogJump(int n, int[] heights, int[] dp) {
        ...
        int left = frogJump(n - 1, heights, dp) + Leetcode.Math.abs(heights[n] - heights[n - 1]);
        int right = Integer.MAX_VALUE;
        if (n > 1) {
            right = frogJump(n - 2, heights, dp) + Leetcode.Math.abs(heights[n] - heights[n - 2]);
        }
        return dp[n] = Leetcode.Math.min(left, right);
    }*/
    ...
    static int frogJump_using_tab(int n, int[] heights) {
        ...
        dp[0] = 0;
        for (int i = 1; i < n; i++) {
            int fs = dp[i - 1] + Leetcode.Math.abs(heights[i] - heights[i - 1]);
            int ss = Integer.MAX_VALUE;
            if (i > 1) {
                ss = dp[i - 2] + Leetcode.Math.abs(heights[i] - heights[i - 2]);
            }
            dp[i] = Leetcode.Math.min(fs, ss);
        }
